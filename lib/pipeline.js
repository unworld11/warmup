...
const SHAPE = `{
  ...
  "content": { "heading": string, "lead": string,
    "tiktoks": [ { "handle": string, "music": string, "caption": string,
      ...
      "stats": { "likes": string, "comments": string, "saves": string, "shares": string } } x3 ],
    "videos": [ { "imagePrompt": string, "duration": "0:32", "title": string, "sub": string } x3 ],
...
- content.tiktoks: 3 slideshows, 3 slides each, modeled on their real formats. slides[].imagePrompt is a photographic prompt (no text/logos in image). overlay/sub are the on-screen text.
- content.videos: 3 short-form video concepts modeled on their real video posts. imagePrompt is the thumbnail scene (photographic, no text/logos), duration is a short clip length like "0:32", title/sub are the on-card labels.
...
export async function generatePage(site, config, imgMap = {}) {
  ...
  const slideshows = tk.map((t) => ({
    cover: img(t.slides?.[0]?.imagePrompt),
    slides: (t.slides || []).length || 5,
    title: t.slides?.[0]?.overlay || t.caption || '',
    sub: t.slides?.[1]?.overlay || t.slides?.[0]?.sub || '',
  })).filter((s) => s.cover);
...
- PALETTE: ${(site.colors && site.colors.length) ? `use these detected colours ${site.colors.join(', ')}` : `use ${site.name}'s well-known official brand colours from your knowledge, plus any colours named in the LAYOUT brief`}. The first strong non-grey is the brand accent; use it prominently and correctly for ${site.name}.
...
- Include ALL of: a top nav (${site.name} name + a small "content proposal" tag + a "Book a call" button); a bold hero (headline, sub, primary and secondary CTA); the insights; a rich content section that renders each slideshow as a card with a small slide-count badge like "1/6" and each video as a thumbnail with a centred circular play button and a duration badge; the reddit thread; the steps as a numbered sequence; and a closing CTA. Use the given image URLs exactly, use the copy verbatim, no placeholder text. Responsive and polished.`;
