...
export async function researchTikTok(handle) {
  try {
    ...
    return items.map((it) => ({
      ...
      isSlideshow: Array.isArray(it.images) ? it.images.length > 0 : Boolean(it.isSlideshow),
...
- content.tiktoks: 4 photo SLIDESHOWS that REPLICATE their real TikTok posts, 4 slides each, each a DIFFERENT named format (set "format") drawn from the formats you see in their real posts. Reuse their REAL @handle in "handle". Set "music" to one of the actual sounds from their real posts when given, otherwise a realistic trending sound. Set "hashtags" to 3 realistic tags in their style (reuse ones they actually use). Set "stats" to realistic numbers in the RANGE of their real posts (likes/comments/saves/shares as short strings like "48.2K"). "overlay" is the exact TikTok text-sticker line that gets RENDERED INTO the slide image, so keep it SHORT, punchy and simple (ideally 3 to 6 words, one line, common words) so the image model spells it cleanly; "sub" is an even shorter second line or empty. Each slide "imagePrompt" describes the ART-DIRECTED photographic SCENE ONLY (the text sticker is added separately, so no text in this scene): name the shot (close-up, wide, over-the-shoulder, POV, flat-lay), the light, the moment, and the emotion, so every slide looks intentional and cinematic, never generic stock. Leave room in the composition (usually the upper third) for the text sticker. No text, words, or logos inside the scene.
...
export async function generatePage(site, config, imgMap = {}) {
  ...
  const slideshows = tk.map((t) => ({
    ...
    slides: (t.slides || [])
      .map((sl) => ({ img: img(slideImagePrompt(sl, config.imageStyle)) }))
      .filter((sl) => sl.img),
